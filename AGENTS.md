...
- New Codex worktrees may start with a minimal shell where `node`, `npx`, `pnpm`, and `corepack` are not on `PATH`.
...
- macOS process inspection commands such as `ps` may be blocked by permissions. Track server sessions started by Codex
  and stop those sessions directly when possible.
