...
<SEO
	title="{profile.name} | {cityConfig.name} Comedy Connector"
	description="{profile.name} — comedian in {cityConfig.name}. {profile.bio?.slice(0, 120) ?? ''}"
	path="/performers/{profile.slug}"
	type="profile"
